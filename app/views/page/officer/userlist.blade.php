			...
			<div class="small-12 large-9 end columns">
				<div class="row">
					<div class="panel side-content">
						...
						<div class="row">
							<div class="columns">
								<a href="{{ URL::to('/user/add') }}" class="button tiny">+&nbsp; Tambahkan</a>
							</div>
						</div>
						<table>
							<thead>
								<tr>
									...
									<th class="large-4">Nama</th>
									<th class="large-4">Username</th>
									<th class="large-1">Enabled</th>
							...
							<tbody>
								...
								<tr>
									...
									<td>{{ $row->name }}</td>
									<td>{{ $row->username }}</td>
									<td><i class='fi-{{ $row->enabled=='1'?'check':'close' }}'></i></td>
									<td>
										<a href="{{URL::to('/user/edit/'.$row->id)}}"><i class="fi-pencil"></i></a>
										<a href="{{URL::to('/user/info/'.$row->id)}}"><i class="fi-info"></i></a>
										<a href="{{URL::to('/user/destroy/'.$row->id)}}"><i class="fi-trash"></i></a>
									</td>
								</tr>
								@endforeach
								@endif
